...
import { getLivestreamTasks, deleteLivestreamTask } from '../api';
...
const getStatusBadge = (status: string) => {
    ...
    if (s === '直播中') return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800 animate-pulse">直播中</span>;
    if (s === 'ai总结' || s === 'processing') return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 animate-pulse">分析中</span>;
    if (s === '已完成' || s === 'completed') return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">已完成</span>;
    if (s === 'failed') return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">失败</span>;
    if (s === '即将开始' || s === 'pending') return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">即将开始</span>;
                ...
                <table className="w-full text-sm text-left text-gray-600">
                    ...
                    <tbody>
                        ...
                        {!isLoading && tasks.map(task => {
                            ...
                            const canViewReport = task.status.toLowerCase() === 'completed' || task.status.toLowerCase() === '已完成';
                            return (
                                <tr key={task.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">{task.host_name || 'N/A'}</td>

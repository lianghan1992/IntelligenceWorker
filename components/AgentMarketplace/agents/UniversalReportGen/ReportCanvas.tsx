...
const IdleState = () => (
    <div className="flex flex-col items-center justify-center h-full text-slate-400 opacity-60">
        ...
        <p className="text-sm mt-2 max-w-xs text-center leading-relaxed">
            输入研究主题，AI 将按顺序为您规划思路、深度检索并撰写万字长文。
        </p>
    </div>
);
...
const SectionStepIndicator: React.FC<{ queries: string[]; count: number; phase: string }> = ({ queries, count, phase }) => (
    <div className="mb-6 p-4 bg-slate-50 border border-indigo-100 rounded-xl animate-in fade-in slide-in-from-top-2 ring-1 ring-indigo-500/10">
...
// 研究思路规划展示
const ReasoningVisualizer: React.FC<{ outline: { title: string; instruction: string }[]; status: string; onConfirm?: () => void }> = ({ outline, status, onConfirm }) => (
    <div className="w-full max-w-3xl mx-auto py-12">
        <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3 mb-8">
            <BrainIcon className="w-7 h-7 text-indigo-600"/> 研究思路规划 (流式解析中)
        </h2>
...
const ReportViewer: React.FC<{ 
    ...
    return (
        <div className="w-full max-w-4xl mx-auto bg-white min-h-full shadow-2xl my-12 p-8 md:p-20 rounded-lg relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500"></div>
            
            <div className="border-b-4 border-slate-900 pb-8 mb-16 text-center">
                <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-4 tracking-tighter uppercase">{topic}</h1>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Seres AI Autonomous Deep Research Report</p>
            </div>
            
            <div className="space-y-24">
                {sections.map((section, idx) => {
                    const isActive = idx === processingIndex;
                    const isCompleted = !isActive && section.content;

                    return (
                        <section 
                            key={idx} 
                            ref={isActive ? activeRef : null}
                            className={`relative transition-all duration-700 ${isActive ? 'scale-[1.02]' : ''} ${!isActive && !section.content ? 'opacity-30' : ''}`}
                        >
                            <div className="flex items-baseline gap-4 mb-8">
                                <span className={`text-6xl font-black select-none leading-none ${isActive ? 'text-indigo-600' : 'text-slate-100'}`}>0{idx + 1}</span>
                                <h2 className={`text-3xl font-black border-b-2 pb-2 flex-1 ${isActive ? 'text-slate-900 border-indigo-500' : 'text-slate-800 border-slate-100'}`}>{section.title}</h2>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                {isActive && !section.content && processingPhase === 'writing' && (
                                    <div className="flex flex-col items-center justify-center py-10 text-indigo-400 gap-3">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-widest animate-pulse">AI 正在根据获取的情报合成正文...</span>
                                    </div>
                                )}
                            </div>
                        </section>
        ...
        <div className="flex-1 h-full overflow-y-auto bg-slate-50 custom-scrollbar relative scroll-smooth">
            <div className="min-h-full">
                ...
                {(status === 'researching' || status === 'planning' || status === 'waiting_confirmation') && (
                    <div className="p-6"><ReasoningVisualizer outline={outline} status={status} onConfirm={onConfirmOutline} /></div>
                )}

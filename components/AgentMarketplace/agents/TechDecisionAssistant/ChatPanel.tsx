...
import { ArrowRightIcon, RefreshIcon, UserIcon, SparklesIcon, CheckCircleIcon, BrainIcon, ChevronDownIcon } from '../../../icons';
...
// Reuse ThinkingBlock from PlanChatArea (or could move to shared)
        ...
        <div className="flex flex-col h-full bg-white border-l border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar" ref={scrollRef}>
                {messages.map((msg, idx) => {
                    ...
                    // Assume the last message if it's assistant and generating is the one streaming reasoning
                    const isLastAssistant = !isUser && idx === messages.length - 1 && isGenerating;
                            ...
                            <div className={`flex flex-col max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}>
                                <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm w-full ${
                                    ...
                                    {!isUser && msg.reasoning && (
                                        <ThinkingBlock content={msg.reasoning} isStreaming={isLastAssistant} />
                                    )}
                ...
                {isGenerating && messages.length > 0 && messages[messages.length - 1].role !== 'assistant' && (
                     // Fallback loading indicator if no assistant message is added yet

...
import { KnowledgeBaseItem, KnowledgeBaseMeta, KnowledgeBaseDetail, InfoItem, SearchResult } from '../../types';
import { getKnowledgeBase, getKnowledgeBaseMeta, getKnowledgeBaseDetail } from '../../api/competitiveness';
import { searchArticlesFiltered } from '../../api/intelligence';
import { 
    RefreshIcon, ChevronDownIcon, CloseIcon, SearchIcon, DocumentTextIcon, CheckCircleIcon, BrainIcon, UsersIcon, LightBulbIcon, TrendingUpIcon, EyeIcon, ClockIcon, ArrowRightIcon
} from '../icons';
...
const DetailPanel: React.FC<{ kbId: number | null; onClose: () => void; }> = ({ kbId, onClose }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm relative">
            {kbId === null && (
                <div className="flex flex-col items-center justify-center h-full text-center text-slate-500">
                    <ArrowRightIcon className="w-12 h-12 text-slate-300 mb-4 -scale-x-100" />
                    <p className="font-semibold">洞察始于探索</p>
                    <p className="text-sm">从左侧选择一个技术情报点，在此处查看其详细演进历史和信源证据。</p>
                </div>
            )}
             {kbId !== null && (
                <>
                    <header className="p-5 border-b flex justify-between items-start flex-shrink-0">
                        {detail ? (
                            <div>
                     ...
                     <main className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
                        {isLoading && <div className="text-center text-gray-500">正在加载详情...</div>}
                        ...
                        {detail && (
                            <>
                                    ...
                                    {sortedTechDetails.map((item, index) => {
                                        ...
                                        return (
                                            <div key={index} className="relative">
                                                <div className="absolute -left-[33px] top-1 w-4 h-4 bg-white border-4 border-slate-300 rounded-full"></div>
                                ...
                                <h3 className="font-semibold text-gray-800 mt-8 mb-4">信源文章列表</h3>
                                <div className="bg-white p-4 rounded-lg border border-gray-200 text-sm text-gray-700 space-y-3 max-h-60 overflow-y-auto">
                                    {articles.length > 0 ? articles.map(article => (
                                        <a href={article.original_url} target="_blank" rel="noopener noreferrer" key={article.id} className="block p-3 bg-slate-50/70 hover:bg-slate-100 rounded-lg">
                                    ...
                                    )) : <p className="text-sm text-center text-gray-500 py-4">无关联信源文章</p>}
                                </div>
                            </>
                        )}
                    </main>
...
export const CompetitivenessDashboard: React.FC = () => {
    ...
    return (
        <div className="h-full flex gap-6 p-6 bg-slate-50 text-gray-800">
            {/* Left Panel */}
            <aside className="w-2/5 flex-shrink-0 flex flex-col h-full">
                <header className="pb-4 flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-gray-800">竞争力看板</h1>
                    <button onClick={() => fetchData()} className="p-2 bg-white border rounded-lg shadow-sm hover:bg-gray-100" title="刷新">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                    ...
                    : Object.entries(groupedData).map(([brand, dims]) => (
                        <div key={brand} className="bg-white rounded-xl border border-gray-200/80 shadow-sm">
                                        ...
                                        return (
                                            <div key={dim} className="flex items-start">
                                                <div className="w-40 flex-shrink-0 flex items-center gap-2 pt-2">
                                                    ...
                                                    <h3 className="font-semibold text-gray-700 text-sm">{dim}</h3>
                                                </div>
                                                            ...
                                                            <div key={item.id} onClick={() => setSelectedKbId(item.id)} className={`group cursor-pointer p-3 rounded-lg border-2 transition-all duration-200 ${isActive ? 'bg-blue-50 border-blue-500 shadow-md' : 'bg-slate-50 border-slate-200/80 hover:shadow-sm hover:border-blue-300'}`}>
                                                                <div className="flex justify-between items-start gap-2">
                                                                    <div>
                                                                        ...
                                                                        <p className={`font-semibold text-gray-800 text-sm truncate group-hover:text-blue-600 ${isActive && 'text-blue-700'}`}>{item.consolidated_tech_preview.name}</p>
                                                                    </div>
            ...
            {/* Right Panel */}
            <main className="w-3/5 flex-shrink-0 h-full">

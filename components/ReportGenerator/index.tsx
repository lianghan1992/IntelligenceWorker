...
const OutlineEditor: React.FC<{ outline: Slide[]; onGenerateContent: () => void }> = ({ outline, onGenerateContent }) => {
    const [visibleItems, setVisibleItems] = useState<Slide[]>([]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            if (currentIndex < outline.length) {
                setVisibleItems(prev => [...prev, outline[currentIndex]]);
                currentIndex++;
                // 自动滚动到底部
        ...
        <div className="max-w-4xl mx-auto animate-in fade-in-0 slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <div className="space-y-4 mb-8">
                {visibleItems.map((slide, index) => (
                    <div key={slide.id} className="bg-white p-6 rounded-xl border border-gray-200 flex items-start gap-5 shadow-sm hover:shadow-md transition-shadow group animate-in slide-in-from-bottom-2 fade-in duration-500">
                ...
                {isStreaming && (
                    <div className="flex items-center justify-center gap-2 py-4 text-indigo-600 opacity-80 animate-pulse" ref={scrollRef}>
                        <SparklesIcon className="w-5 h-5" />
                        <span className="text-sm font-medium">AI 正在规划下一章...</span>
                    </div>
                )}
            </div>

            <div className={`bg-gradient-to-br from-white to-indigo-50 p-6 rounded-2xl border border-indigo-100 shadow-lg sticky bottom-6 transition-all duration-500 ${isStreaming ? 'opacity-50 grayscale pointer-events-none' : 'opacity-100'}`}>
                ...
                <div className="relative">
                    ...
                    <div className="absolute bottom-3 right-3">
                        <button 
                            ...
                            disabled={isStreaming}
                            className="px-6 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg shadow-md hover:bg-indigo-700 transition transform hover:scale-105 flex items-center gap-2 disabled:opacity-50 disabled:scale-100"
                        >

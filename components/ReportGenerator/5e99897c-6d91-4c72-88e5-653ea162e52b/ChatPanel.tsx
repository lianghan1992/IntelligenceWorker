...
interface Attachment {
    id: string;
    type: 'image' | 'file' | 'web'; // Added 'web'
    content: string; 
    name: string;
    size: number;
    isTruncated?: boolean;
    tokenEstimate?: number;
    url?: string; // For web type
}
    ...
    return (
        <div className={`flex gap-4 animate-in fade-in slide-in-from-bottom-2 ${isUser ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[85%] rounded-2xl overflow-hidden shadow-sm transition-all duration-300 ${
                isUser 
                    ? 'bg-gradient-to-br from-indigo-600 to-blue-700 text-white rounded-tr-sm p-5 shadow-indigo-200' 
                    : isCodeMode 
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ 
    ...
    const handleWebFetchConfirm = (fetchedItems: Attachment[]) => {
        setAttachments(prev => [...prev, ...fetchedItems]);
    };
    ...
    const handleSend = () => {
        ...
        // Construct hidden prompt injection for attachments
    ...
    return (
        <div className="flex flex-col h-full bg-white relative">
            ...
            <div className="bg-white border-t border-slate-100 shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.05)] z-20">
                ...
                <div className="relative px-4 pb-4">
                    <textarea
                        ...
                        placeholder={stage === 'analysis' ? "输入技术名称，开始分析... (Shift+Enter 换行)" : "描述修改要求... (Shift+Enter 换行)"}
            ...
            <WebFetchModal isOpen={isWebModalOpen} onClose={() => setIsWebModalOpen(false)} onConfirm={handleWebFetchConfirm} />
        </div>
    );
};

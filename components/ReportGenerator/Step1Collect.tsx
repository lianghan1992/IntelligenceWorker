    ...
    return (
        <div className="h-full flex flex-col p-8 overflow-y-auto custom-scrollbar bg-white">
            <div className="max-w-3xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-indigo-500/20 shadow-sm"
                            />
                            <button 
                                onClick={handleAddUrl}
                                disabled={isFetching || !urlInput.trim()}
                                className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 shadow-sm transition-colors"
                            >
                                {isFetching ? <RefreshIcon className="w-5 h-5 animate-spin" /> : <GlobeIcon className="w-5 h-5" />}
                            </button>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => setIsKMOpen(true)} className="flex-1 py-2 bg-white text-slate-600 rounded-lg text-xs font-bold hover:text-indigo-600 hover:border-indigo-200 transition-all flex items-center justify-center gap-2 border border-slate-200 shadow-sm">
                                <PuzzleIcon className="w-4 h-4" /> 知识库
                            </button>
                            <label className="flex-1 py-2 bg-white text-slate-600 rounded-lg text-xs font-bold hover:text-indigo-600 hover:border-indigo-200 transition-all flex items-center justify-center gap-2 border border-slate-200 shadow-sm cursor-pointer">
                    ...
                    <div className="pt-6 border-t border-slate-200">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                            {materials.length === 0 ? (
                                <div className="py-12 flex flex-col items-center justify-center text-slate-400 text-xs italic border-2 border-dashed border-slate-200 rounded-xl">
                                    暂无资料引用，AI 将仅根据通用知识创作
                                </div>
                            ) : (
                                materials.map(m => (
                                    <div key={m.id} className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200 group shadow-sm">
                ...
                <div className="bg-gradient-to-br from-indigo-900 to-indigo-800 rounded-2xl p-8 text-white relative overflow-hidden shadow-xl shadow-indigo-200/50">

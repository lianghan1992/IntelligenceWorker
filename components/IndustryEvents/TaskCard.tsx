...
interface TaskCardProps {
    ...
    onViewReport: () => void;
    index: number; // Added to handle sequential loading
}
...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onViewReport, index }) => {
    ...
    // 顺序加载逻辑：根据 index 设置延迟
    useEffect(() => {
        if (!task.id) return;
        const timer = setTimeout(() => {
            setShouldFetch(true);
        }, index * 80 + 300); // 300ms 基础延迟，等待文字和 Hero 渲染
        return () => clearTimeout(timer);
    }, [task.id, index]);

    useEffect(() => {
        if (!shouldFetch || !task.id) return;
        let active = true;
    ...
    return (
        <div
            ...
            className={`group relative aspect-[16/10] w-full overflow-hidden rounded-2xl bg-slate-100 shadow-md transition-all duration-700 ${hasReport ? 'cursor-pointer' : 'cursor-default'} ${isImageLoaded ? 'opacity-100 translate-y-0 shadow-lg' : 'opacity-80 translate-y-2'}`}
        >
            {imageUrl ? (
                <img 
                    src={imageUrl} 
            ...
            <span className={`absolute top-3 right-3 px-3 py-1 text-xs font-bold rounded-full z-20 ${statusDetails.className}`}>{statusDetails.text}</span>
            {statusDetails.type === 'upcoming' && timeLeft && timeLeft !== "即将开始" && <CountdownDisplay timeLeft={timeLeft} />}
            <div className="absolute inset-0 flex flex-col justify-end p-4 text-white z-10">
                <div className="text-lg font-bold drop-shadow-md leading-tight group-hover:text-indigo-300 transition-colors">{task.task_name}</div>
                <p className="text-xs text-gray-300 mt-1.5 drop-shadow-sm font-medium">{task.company} · {formattedDate}</p>
                {isLive && (
                    <div className="mt-4">
                        <a href={task.live_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold bg-red-600 text-white rounded-lg shadow-md hover:bg-red-700 transition-all transform hover:scale-105 active:scale-95"><PlayIcon className="w-4 h-4" /><span>观看直播</span></a>
                    </div>
                )}
                {isFinished && (
                    <div className="mt-4 flex gap-3">
                        <button onClick={hasReport ? onViewReport : undefined} disabled={!hasReport} className="flex items-center justify-center gap-2 px-4 py-2 text-xs font-semibold bg-white text-slate-900 rounded-lg hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm active:scale-95"><DocumentTextIcon className="w-4 h-4" /><span>{hasReport ? '查看报告' : '报告生成中'}</span></button>
                        <button onClick={handleReplayClick} className="flex items-center justify-center gap-2 px-4 py-2 text-xs font-semibold bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-white/20 transition-colors active:scale-95"><FilmIcon className="w-4 h-4" /><span>回放</span></button>
                    </div>
                )}
            </div>
        </div>

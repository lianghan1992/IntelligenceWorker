...
import { UploadedDocument, SpiderPoint } from '../../../types';
import { getUploadedDocs, getSpiderPoints, downloadUploadedDoc } from '../../../api/intelligence';
import { 
    CloudIcon, RefreshIcon, SearchIcon, FilterIcon, CalendarIcon, 
    DownloadIcon, ArrowRightIcon, EyeIcon, PlusIcon
} from '../../icons';
...
export const DocumentManager: React.FC = () => {
    ...
    // Selection for Batch Move (Optional: currently move modal supports moving all from a point or specific IDs)
    // For now, let's keep it simple: Filter by point -> Move all from this point to another
    
    const fetchPoints = async () => {
        try {
            const res = await getSpiderPoints();
            setPoints(res);
    ...
    const fetchDocs = useCallback(async () => {
        ...
        try {
            const res = await getUploadedDocs({
                ...
                point_uuid: pointFilter || undefined,
    ...
    }, [page, search, pointFilter, startDate, endDate]);
        ...
        <div className="h-full flex flex-col bg-white">
            ...
            <div className="p-4 border-b border-gray-100 flex flex-col gap-4 bg-white z-10">
                <div className="flex justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <CloudIcon className="w-6 h-6 text-indigo-600" /> 上传文档库
                    </h3>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 px-2 py-1 rounded-lg border border-gray-200">
                        ...
                        <select 
                            value={pointFilter} 
                            onChange={e => { setPointFilter(e.target.value); setPage(1); }}
                            className="bg-transparent text-sm text-gray-700 outline-none border-none min-w-[120px]"
                        >
                    ...
                    <button onClick={() => fetchDocs()} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500 border border-transparent hover:border-gray-200">
                    ...
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b">
                            <tr>
                                ...
                                <th className="px-6 py-3 font-medium w-48">所属采集点</th>
                                <th className="px-6 py-3 font-medium w-32">页数</th>
                                ...
                                docs.map(doc => (
                                    <tr key={doc.uuid} className="hover:bg-slate-50 transition-colors group">
                                        <td className="px-6 py-4 font-medium text-gray-900">
                                            <div className="truncate max-w-xs" title={doc.original_filename}>{doc.original_filename}</div>
                                        </td>
                                        <td className="px-6 py-4 text-xs">
                                            <div>{formatSize(doc.file_size)}</div>
                                            <div className="text-gray-400 uppercase">{doc.mime_type.split('/')[1] || 'FILE'}</div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">{doc.page_count} P</td>
            ...
            {isUploadModalOpen && (
                <DocUploadModal 
                    ...
                    onSuccess={() => { fetchDocs(); setIsUploadModalOpen(false); }}
                    points={points}
                />
            )}
            ...
            {isMoveModalOpen && (
                <DocMoveModal 
                    ...
                    onSuccess={() => { fetchDocs(); setIsMoveModalOpen(false); }}
                    points={points}
                />
            )}

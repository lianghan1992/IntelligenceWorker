...
import { CloseIcon, PlusIcon, TrashIcon, DownloadIcon, ServerIcon, SearchIcon, CalendarIcon, BrainIcon, FilterIcon } from '../../icons';
...
const KeywordBuilder: React.FC<{ value: string; onChange: (val: string) => void }> = ({ value, onChange }) => {
    // Parse initial value: assume format "AND1 AND2 OR1|OR2|OR3"
    // Heuristic: split by space. If part contains '|', it's an OR group. Else it's an AND item.
    // Note: This is a simplified view.
    
    const [andTags, setAndTags] = useState<string[]>([]);
    const [orTags, setOrTags] = useState<string[]>([]);
    
    // Initialize state from prop value only once or when value changes externally (complex to track, 
    // better to just parse on mount and manage internally, then emit string)
    // Actually, to support clearing/resetting from parent, we might need a key or effect.
    // For simplicity in this modal, we initialize once.
    ...
    useEffect(() => {
        ...
        if (value) {
            ...
            parts.forEach(p => {
                if (p.includes('|')) {
                    p.split('|').forEach(o => o && ors.push(o));
                } else {
                    ands.push(p);
                }
            });
    ...
    const emitChange = (newAnds: string[], newOrs: string[]) => {
        const andPart = newAnds.join(' ');
        const orPart = newOrs.join('|');
        const final = [andPart, orPart].filter(Boolean).join(' ');
        onChange(final);
    };

    const addAndTag = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            const val = e.currentTarget.value.trim();
            if (val && !andTags.includes(val)) {
                const newTags = [...andTags, val];
                setAndTags(newTags);
                emitChange(newTags, orTags);
    ...
    const removeAndTag = (tag: string) => {
        const newTags = andTags.filter(t => t !== tag);
        setAndTags(newTags);
        emitChange(newTags, orTags);
    };

    const addOrTag = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            const val = e.currentTarget.value.trim();
            if (val && !orTags.includes(val)) {
                const newTags = [...orTags, val];
                setOrTags(newTags);
                emitChange(andTags, newTags);
                e.currentTarget.value = '';
            }
        }
    };
    ...
    const removeOrTag = (tag: string) => {
        const newTags = orTags.filter(t => t !== tag);
        setOrTags(newTags);
        emitChange(andTags, newTags);
    };
        ...
        <div className="space-y-3 bg-white border border-gray-200 rounded-lg p-3">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                    {andTags.map(tag => (
                        <span key={tag} className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 border border-blue-100">
                            {tag}
                            <button onClick={() => removeAndTag(tag)} className="hover:text-blue-900"><CloseIcon className="w-3 h-3" /></button>
                        </span>
                    ))}
                    <input 
                        className="flex-1 min-w-[120px] bg-transparent text-xs outline-none placeholder:text-slate-300 py-1"
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {orTags.map(tag => (
                        <span key={tag} className="bg-purple-50 text-purple-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 border border-purple-100">
                            {tag}
                            <button onClick={() => removeOrTag(tag)} className="hover:text-purple-900"><CloseIcon className="w-3 h-3" /></button>
                        </span>
                    ))}
                    <input 
                        className="flex-1 min-w-[120px] bg-transparent text-xs outline-none placeholder:text-slate-300 py-1"
    ...
    return (
        <div className="p-4 border border-slate-200 rounded-xl bg-slate-50/50 relative group">
            ...
            <div className="flex items-center gap-2 mb-4">
                <div className="w-6 h-6 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-xs font-bold text-slate-500 shadow-sm">
                    {index + 1}
                </div>
                <input 
                    type="text" 
                    ...
                    className="bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 text-sm font-bold text-slate-800 outline-none px-1 transition-colors w-48 placeholder:font-normal"
                    placeholder="任务名称 (必填)"
                />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                {/* Left Col: Query Config */}
                <div className="space-y-3">
                    <KeywordBuilder 
                        value={task.keywords} 
                        onChange={(val) => onChange(task.id, 'keywords', val)}
                    />
                </div>
                ...
                <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">数据来源</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">时间范围</label>
                        <div className="flex gap-2 items-center bg-white border border-slate-200 rounded-lg p-1">
                            ...
                            <input 
                                ...
                                className="flex-1 bg-transparent py-1 text-xs outline-none text-slate-600" 
                            />
            ...
            {/* Footer: Advanced Params */}
            <div className="flex items-center gap-4 pt-3 border-t border-slate-200/60">
                <label className="flex items-center gap-2 cursor-pointer select-none group">
                    <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${task.use_vector_search ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-300 group-hover:border-indigo-400'}`}>
                        {task.use_vector_search && <PlusIcon className="w-3 h-3 text-white transform rotate-45" />} 
                        {/* Using Plus rotated as check or simplify check icon */}
                    </div>
                    <input 
                        type="checkbox" 
                        className="hidden"
                    ...
                    <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-900">
                        <BrainIcon className="w-3.5 h-3.5 text-indigo-500" /> 向量增强
                    </div>
                </label>
                
                {task.use_vector_search && (
                    <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2">
                        <span className="text-[10px] text-slate-400 font-bold uppercase">阈值</span>
                        <input 
                            type="number" 
                            min="0.1" max="1.0" step="0.1" 
                            value={task.similarity_threshold}
                            onChange={e => onChange(task.id, 'similarity_threshold', parseFloat(e.target.value))}
                            className="w-12 bg-white border border-slate-200 rounded px-1 py-0.5 text-xs text-center outline-none focus:border-indigo-400 font-mono"
                        />
                    </div>
                )}
                ...
                <div className="flex items-center gap-2">
                    <span className="text-[10px] text-slate-400 font-bold uppercase">最大条数</span>
                    <input 
                        type="number" 
                        min="1" max="5000" step="50" 
                        value={task.limit}
                        onChange={e => onChange(task.id, 'limit', parseInt(e.target.value))}
                        className="w-16 bg-white border border-slate-200 rounded px-1 py-0.5 text-xs text-center outline-none focus:border-indigo-400 font-mono"
                    />
                </div>
            </div>
        </div>
...
export const BatchSearchExportModal: React.FC<BatchSearchExportModalProps> = ({ isOpen, onClose, sources }) => {
    ...
    return (
        // Changed overlay to be much lighter/transparent
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-white/60 backdrop-blur-sm animate-in fade-in zoom-in-95">

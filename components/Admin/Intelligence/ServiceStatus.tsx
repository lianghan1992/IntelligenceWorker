...
import { RefreshIcon, CheckCircleIcon, ShieldExclamationIcon, PlusIcon, TrashIcon, PlayIcon, ServerIcon } from '../../icons';
...
export const ServiceStatus: React.FC = () => {
    ...
    const [newProxy, setNewProxy] = useState({ url: '', note: '' });
    ...
    const handleAddProxy = async () => {
        ...
        try {
            await addProxy({ url: newProxy.url, enabled: true, note: newProxy.note });
            setNewProxy({ url: '', note: '' });
    ...
    const handleTestProxy = async (url: string) => {
        ...
        try {
            ...
            setProxies(prev => prev.map(p => {
                if (p.url === url) {
                    return { ...p, latency_ms: res.success ? res.latency_ms : -1 }; // -1 indicates failure for UI
                }
                return p;
            }));
        } catch (e) {
                    ...
                    <div className="p-4 bg-indigo-50 border-b border-indigo-100 animate-in slide-in-from-top-2">
                        <div className="flex flex-col md:flex-row gap-3">
                            ...
                            <input 
                                ...
                                placeholder="备注 (可选)" 
                                value={newProxy.note}
                                onChange={e => setNewProxy({...newProxy, note: e.target.value})}
                                className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                            />
                            <button 
                    ...
                    <table className="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {isLoadingProxies && proxies.length === 0 ? (
                                ...
                                proxies.map((proxy) => {
                                    const protocol = proxy.url.split('://')[0] || 'http';
                                    return (
                                        <tr key={proxy.url} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-mono text-gray-800">{proxy.url}</td>
                                            <td className="px-6 py-4"><span className="uppercase text-xs font-bold bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{protocol}</span></td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <span className={`w-2 h-2 rounded-full ${proxy.enabled ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                                                    {proxy.latency_ms !== undefined && (
                                                        <span className={`text-xs font-mono px-1.5 py-0.5 rounded ${proxy.latency_ms >= 0 && proxy.latency_ms < 500 ? 'bg-green-50 text-green-700' : proxy.latency_ms >= 500 ? 'bg-yellow-50 text-yellow-700' : 'bg-red-50 text-red-700'}`}>
                                                            {proxy.latency_ms >= 0 ? `${proxy.latency_ms}ms` : 'Timeout'}
                                                        </span>
                                                    )}
                                                </div>
                                            </td>

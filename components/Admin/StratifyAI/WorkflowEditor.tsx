...
export const WorkflowEditor: React.FC<WorkflowEditorProps> = ({ value, onChange }) => {
    ...
    return (
        <div className="flex flex-col h-full border border-slate-200 rounded-2xl overflow-hidden bg-slate-50">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {config.variables.map((v, idx) => (
                                <div key={idx} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-3">
                                    <div className="flex gap-3">
                                        <div className="flex-1">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">Variable Name</label>
                                            <input value={v.name} onChange={e => updateVariable(idx, 'name', e.target.value)} className="w-full text-sm font-mono border-b border-slate-200 focus:border-indigo-500 outline-none py-1" placeholder="e.g. topic" />
                                        </div>
                                        <div className="flex-1">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">Label</label>
                                            <input value={v.label} onChange={e => updateVariable(idx, 'label', e.target.value)} className="w-full text-sm border-b border-slate-200 focus:border-indigo-500 outline-none py-1" placeholder="e.g. 研究主题" />
                                        </div>
                                        <div className="w-24">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">Type</label>
                                            <select value={v.type} onChange={e => updateVariable(idx, 'type', e.target.value)} className="w-full text-sm border-b border-slate-200 focus:border-indigo-500 outline-none py-1 bg-transparent">
                                                {VARIABLE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                                            </select>
                                        </div>
                                        <button onClick={() => removeVariable(idx)} className="text-slate-300 hover:text-red-500 self-end mb-1"><TrashIcon className="w-4 h-4" /></button>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                                            <input type="checkbox" checked={v.required} onChange={e => updateVariable(idx, 'required', e.target.checked)} className="rounded text-indigo-600" />
                                            必填
                                        </label>
                                        {v.type === 'select' && (
                                            <input value={v.options?.join(',') || ''} onChange={e => updateVariable(idx, 'options', e.target.value.split(','))} className="flex-1 text-xs border border-slate-200 rounded px-2 py-1 outline-none" placeholder="选项 (逗号分隔)" />
                                        )}
                                        <input value={v.default || ''} onChange={e => updateVariable(idx, 'default', e.target.value)} className="flex-1 text-xs border border-slate-200 rounded px-2 py-1 outline-none" placeholder="默认值 (可选)" />
                                    </div>
                                </div>
                            ))}
                            {config.variables.length === 0 && <div className="text-center py-8 text-slate-400 text-xs border border-dashed rounded-xl">暂无变量</div>}
                        </div>
                    </div>
                )}
                                ...
                                <div key={idx} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="w-6 h-6 rounded-full bg-white border border-slate-200 flex items-center justify-center text-xs font-bold text-slate-500">{idx + 1}</div>
                                            <div>
                                                <div className="text-sm font-bold text-slate-700">{step.name}</div>
                                                <div className="text-[10px] text-slate-400 font-mono">{step.id}</div>
                                            </div>
                                            {step.type && <span className="text-[10px] px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded border border-indigo-100 uppercase">{step.type}</span>}
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <button onClick={(e) => { e.stopPropagation(); removeStep(idx); }} className="p-1 hover:bg-red-50 text-slate-400 hover:text-red-500 rounded"><TrashIcon className="w-4 h-4" /></button>
                                            <button className="p-1 text-slate-400">{expandedStepIdx === idx ? <ChevronDownIcon className="w-4 h-4"/> : <ChevronRightIcon className="w-4 h-4"/>}</button>
                                        </div>
                                    </div>

                                    {/* Expanded Detail */}
                                    {expandedStepIdx === idx && (
                                        <div className="p-4 border-t border-slate-100 space-y-4 bg-white animate-in slide-in-from-top-2 fade-in">
                                            
                                            {/* Basic Info */}
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="label-xs">ID</label>
                                                    <input value={step.id} onChange={e => updateStep(idx, 'id', e.target.value)} className="input-sm font-mono" />
                                                </div>
                                                <div>
                                                    <label className="label-xs">Name</label>
                                                    <input value={step.name} onChange={e => updateStep(idx, 'name', e.target.value)} className="input-sm" />
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <select value={step.type} onChange={e => updateStep(idx, 'type', e.target.value)} className="input-sm">
                                                        {STEP_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                                                    </select>
                                                </div>
                                                <div>
                                                    <label className="label-xs">Condition (JS Expression)</label>
                                                    <input value={step.condition || ''} onChange={e => updateStep(idx, 'condition', e.target.value)} className="input-sm font-mono text-orange-600" placeholder="e.g. !inputs.has_outline" />
                                                </div>
                                            </div>
                                            ...
                                            {step.type === 'generation' && (
                                                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-3">
                                                    <div className="flex justify-between items-center"><span className="text-xs font-bold text-slate-500">LLM 配置</span></div>
                                                    
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div>
                                                            <label className="label-xs">Prompt Template</label>
                                                            <select value={step.prompt_id || ''} onChange={e => updateStep(idx, 'prompt_id', e.target.value)} className="input-sm">
                                                    ...
                                                    {/* Input Mapping */}
                                                    <div>
                                                        <label className="label-xs mb-1 block">Input Mapping (JSON)</label>
                                                        <textarea 
                                                            ...
                                                            className="w-full h-24 bg-white border border-slate-200 rounded-lg p-2 text-xs font-mono resize-none focus:ring-1 focus:ring-indigo-500 outline-none"
                                                            placeholder='{ "topic": "{{inputs.topic}}" }'
                                                        />
                                                        <p className="text-[10px] text-slate-400 mt-1">Use <code>{`{{ inputs.var }}`}</code> or <code>{`{{ steps.step_id.result }}`}</code></p>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                 <div>
                                                    <label className="label-xs">UI Component</label>
                                                    <select value={step.ui?.component || 'MarkdownEditor'} onChange={e => updateStepNested(idx, 'ui', 'component', e.target.value)} className="input-sm">
                                                        {UI_COMPONENTS.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                                                    </select>
                                                </div>
                                                <div className="flex items-center gap-2 mt-4">
                                                    <input type="checkbox" checked={step.ui?.editable !== false} onChange={e => updateStepNested(idx, 'ui', 'editable', e.target.checked)} className="rounded text-indigo-600" />
                                                    <span className="text-xs text-slate-600 font-medium">允许用户修改结果</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
            ...
            <style>{`
                .label-xs { @apply block text-[10px] font-bold text-slate-400 uppercase mb-1; }
                .input-sm { @apply w-full bg-white border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:ring-1 focus:ring-indigo-500 outline-none transition-shadow; }
            `}</style>
        </div>
    );
};

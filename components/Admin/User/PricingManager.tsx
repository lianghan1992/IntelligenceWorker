...
import { getModelPricings, createModelPricing, deleteModelPricing } from '../../../api/user';
import { getChannels } from '../../../api/stratify'; // Channels are still in Stratify
import { 
...
export const PricingManager: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [pData, cData] = await Promise.all([getModelPricings(), getChannels()]);
            setPricings(pData);
            setChannels(cData);
        } catch (e) {
            console.error(e);

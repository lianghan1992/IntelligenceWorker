...
- 当前周期按 30 天计算
- 当订阅为 active 且未过期时：以订阅到期日 `expires_at` 作为当前周期结束时间（周期开始为 `expires_at - 30 天`）
- 在其它场景下：周期以订阅更新日 `subscription.updated_at`（没有则退到 created_at）为锚点，从锚点日起每 30 天一个周期
...
- 权益不足：HTTP 403 或 402（你可以统一用 403，body 里带原因）
- 余额不足：提示充值（引导到充值页面）
- 无配置：属于运营配置问题，建议提示“功能暂不可用/请联系管理员”

### 4.3 当前实现的一些重要行为（避免踩坑）

...
注意：`overage_unit_price` 的单位是“resource_key 的单位价格”。例如 `ppt_pages` 的单价就是“每页多少钱”。
...
为了支持“配额按页扣，超额按 Token 扣”等复杂场景，以及“生成前预判”需求，consume 接口新增了两个参数：
...
1) **混合计费 (`billing_count`)**
   ...
   - 逻辑：
     ...
     - 如果配额不足（走超额）：忽略 `count`，按 `billing_count * overage_unit_price` 计算扣费金额。
     - 注意：`overage_unit_price` 需配置为“Token 单价”。
...
1. **预判**：调用 consume (check_only=true, billing_count=预估Token)。若 allowed=false，直接提示用户充值。
2. **执行**：调用大模型生成内容，获取实际消耗 Token。
3. **结算**：调用 consume (check_only=false, count=实际页数, billing_count=实际Token)。

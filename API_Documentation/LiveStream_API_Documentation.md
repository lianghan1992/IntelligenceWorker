...
## 轻量化任务列表
- 接口介绍：**全新接口**，专为首屏快速加载设计。仅返回任务的基础元数据（ID、名称、车企、状态），不包含 `stats_json`、`cover_image_b64` 等大字段。
- 接口方法：`GET /api/livestream/tasks/lite`
- 字段说明（查询参数）：
  - `page`：整数，默认 `1`。
  - `size`：整数，默认 `20`。
  - `company`：字符串，可选，按车企名称模糊筛选。
- curl 示例：
```bash
curl -sS 'http://127.0.0.1:7651/api/livestream/tasks/lite?page=1&size=20'
```
- 返回示例：
```json
{
  ...
  "items": [
    {
      "id": "66c0e799-d0e7-4de6-95ea-dbd45022d11a",
      "task_name": "B站直播测试",
      "company": "示例车企",
      "status": "recording"
    }
...
## 获取任务封面
- 接口介绍：获取指定任务的封面图片（JPEG 格式二进制流），可直接用于 `<img src="...">`。
- 接口方法：`GET /api/livestream/tasks/{task_id}/cover`
- 字段说明：无请求体。
- curl 示例：
```bash
curl -sS http://127.0.0.1:7651/api/livestream/tasks/<task_id>/cover > cover.jpg
```
- 返回示例：
  - HTTP 200 OK

...
export const StrategicCockpit: React.FC<{ subscriptions: SystemSource[] }> = ({ subscriptions }) => {
    ...
    const fetchArticles = useCallback(async (page: number = 1) => {
        ...
        // Get current query state from closure
        const { value: queryValue, label: queryLabel, type: queryType } = activeQuery;
        ...
        try {
            ...
            // Check if we should use Tag Search for Industry and Customer categories
            // The tag is the sub-category label (e.g. "新技术", "政策")
            if (queryType === 'sublook' && (look === 'industry' || look === 'customer')) {
                // Use tag search with the label as tag
                response = await getArticlesByTags([queryLabel], page, limit);
            } else if (queryValue === '*') {
                // All Intelligence
                const params: any = {
                    page,
                ...
                // Use semantic search for Competitor/Self categories, POIs, or manual search
                // These use the keywords string defined in data.ts or search input
    ...
    }, [activeQuery, selectedLook]); 


    useEffect(() => {
        // Trigger fetch when activeQuery or selectedLook changes (reset to page 1)
        fetchArticles(1);
    }, [fetchArticles]);

...
export async function extractKeywords(text: string): Promise<string[]> {
    ...
    try {
        ...
        const stopWords = new Set([
            ...
            '个', '么', '之', '其', '其二', '其一', '具有', '的', '地', '得', '着', '了', '过', '把', '被', '向', '从', '由', '往', '在', '至', '于', '以', '用',
            '上', '下', '左', '右', '前', '后', '内', '外', '中', '间', '里', '旁', '这', '那', '哪',
            '一个', '一些', '这个', '那个', '什么', '怎么', '不是',
            '进行', '表示', '通过', '而且', '但是', '所以', '因此', '如果', '那么',
            '情况', '问题', '方面', '一种', '一样', '一般', '这样', '那样',
            '然而', '同时', '另外', '于是', '然后', '最后', '其次', '首先',
            '很', '太', '最', '极', '非常', '相当', '十分', '尤其', '并且',
        ...
        const filteredWords = words.filter(word => {
            ...
            return trimmed.length > 1 && !stopWords.has(trimmed) && !/^\d+$/.test(trimmed);
        });
